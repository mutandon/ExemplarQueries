 ...
 */
package eu.unitn.disi.db.exemplar.isomorphism.core;
 ...
 */
public class IsomorphicAnswer extends ExemplarAnswer {
     ...
     */
    public IsomorphicAnswer(Multigraph query) {
        super(query);
        this.initialize();
    }
    ...
    @Override
    public IsomorphicAnswer getClone() {
        IsomorphicAnswer clone = new IsomorphicAnswer(this.query);
    ...
    /**
     *
     * @return the list of graph nodes mapped to some query node
     */
    @Override
    ...
    public boolean isUsing(Edge graphEdge) {
        return  this.mappedEdges.values().contains(graphEdge);
    }
    ...
    public Set<String> getUsedEdgesIDs() {
        ...
        for (Edge e  : this.mappedEdges.values()) {
            m.add(e.getId());
        }        
        return m;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof IsomorphicAnswer) {
            final IsomorphicAnswer other = (IsomorphicAnswer) obj;
...
//            }
            ...
            if (other.mappedEdges.size() == this.mappedEdges.size()) {
                for (Edge otherEdge : other.mappedEdges.values()) {
                    if (!this.isUsing(otherEdge)) {
                        return false;
                    }
                }
                return true;
    ...
    @Override
    /**
     * The size will be always the size
     */
    public int size(){
        return this.mappedEdges.values().stream().filter(e -> Objects.nonNull(e)).collect(Collectors.toSet()).size();
    }
}
